...
namespace SharpLearning.GradientBoost.Test.Learners
{
    ...
    public class RegressionLeastAbsoluteDeviationGradientBoostLearnerTest
    {
        ...
        [TestMethod]
        public void RegressionLeastAbsoluteDeviationGradientBoostLearner_Stochastic_Learn_AptitudeData()
        {
            var parser = new CsvParser(() => new StringReader(Resources.AptitudeData));
            var observations = parser.EnumerateRows(v => v != "Pass").ToF64Matrix();
            var targets = parser.EnumerateRows("Pass").ToF64Vector();

            var sut = new RegressionLeastAbsoluteDeviationGradientBoostLearner(100, 0.1, 3, 2000, 1, 0.000001, 0.5);
            ...
            Assert.AreEqual(0.39406952041807586, actual);
        }
