...
namespace SharpLearning.Metrics.Test.Classification
{
    ...
    public class RocAucClassificationMetricTest
    {
        ...
        public void RocAucClassificationMetric_Error_No_Error()
        {
            ...
            var probabilities = new double[] { 0, 1 };
        ...
        public void RocAucClassificationMetric_Error_Random()
        {
            ...
            var probabilities = Enumerable.Range(0, 1000)
                .Select(s => random.NextDouble()).ToArray();

            var sut = new RocAucClassificationMetric<double>(1);
            var actual = sut.Error(targets, probabilities);

            Assert.AreEqual(0.50509375000000012, actual, 0.0001);
        }
