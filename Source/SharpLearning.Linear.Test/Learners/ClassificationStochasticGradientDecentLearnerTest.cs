...
namespace SharpLearning.Linear.Test.Learners
{
    ...
    public class ClassificationStochasticGradientDecentLearnerTest
    {
        ...
        [TestMethod]
        public void ClassificationStochasticGradientDecentLearner_Learn_Binary_Indexed()
        {
            var parser = new CsvParser(() => new StringReader(Resources.LogitTest));
            var observations = parser.EnumerateRows("F1", "F2").ToF64Matrix();
            var targets = parser.EnumerateRows("Target").ToF64Vector();
            var indices = Enumerable.Range(0, 75).ToArray();
            ...
            var actualImportances = model.GetRawVariableImportance();
            Assert.AreEqual(0.13, actualError, 0.001);
            Assert.AreEqual(0.13003604638851227, actualImportances[0], 0.001);
            Assert.AreEqual(0.10930472561412716, actualImportances[1], 0.001);
        }
        ...
        [TestMethod]
        public void ClassificationStochasticGradientDecentLearner_Learn_Multi_Indexed()
        {
            var parser = new CsvParser(() => new StringReader(Resources.Glass));
            var observations = parser.EnumerateRows(s => s != "Target").ToF64Matrix();
            var targets = parser.EnumerateRows("Target").ToF64Vector();
            var indices = Enumerable.Range(0, 190).ToArray();
            ...
            Assert.AreEqual(0.58878504672897192, actualError, 0.001);
        }
