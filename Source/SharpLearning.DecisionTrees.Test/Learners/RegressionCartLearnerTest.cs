...
namespace SharpLearning.DecisionTrees.Test.Learners
{
    ...
    public class RegressionCartLearnerTest
    {
        ...
        [TestMethod]
        public void RegressionCartLearner_Learn_Depth_100_Weight_1()
        {
            var error = RegressionCartLearner_Learn_Weighted(100, 1.0);
            Assert.AreEqual(0.038873687234849852, error, 0.0000001);
        }

        [TestMethod]
        public void RegressionCartLearner_Learn_Depth_1_Weight_1()
        {
            var error = RegressionCartLearner_Learn_Weighted(1, 1);
            Assert.AreEqual(0.55139468272009107, error, 0.0000001);
        }

        [TestMethod]
        public void RegressionCartLearner_Learn_Depth_3_Weight_1()
        {
            var error = RegressionCartLearner_Learn_Weighted(2, 1);
            Assert.AreEqual(0.14322350107327153, error, 0.0000001);
        }

        [TestMethod]
        public void RegressionCartLearner_Learn_Depth_100_Weight_100()
        {
            var error = RegressionCartLearner_Learn_Weighted(100, 100.0);
            Assert.AreEqual(0.14322350107327153, error, 0.0000001);
        }
        ...
        private double RegressionCartLearner_Learn_Weighted(int treeDepth, double weight)
        {
            var parser = new CsvParser(() => new StringReader(Resources.DecisionTreeData));
            var observations = parser.EnumerateRows("F1", "F2").ToF64Matrix();
            var targets = parser.EnumerateRows("T").ToF64Vector();
