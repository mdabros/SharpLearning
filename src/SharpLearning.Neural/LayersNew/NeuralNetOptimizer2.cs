...
namespace SharpLearning.Neural.LayersNew
{
    ...
    /// </summary>
    public sealed class NeuralNetOptimizer2
    {
        double m_learningRate;
        readonly double m_learningRateInit;
        ...
        readonly double m_rho = 0.95f;
        readonly double m_eps = 1e-8f;
        readonly double m_beta1 = 0.9f;
        readonly double m_beta2 = 0.999f;

        // Nadam specific members.
        double m_schedule_decay = 0.004f;
        double m_schedule = 1.0f;
        double m_momentumCache;
        double m_momentumCache_1;
        double m_scheduleNew;
        double m_scheduleNext;

        readonly double m_l1Decay = 0.0f;
        readonly double m_l2Decay = 0.0f;
        ...
        public NeuralNetOptimizer2(double learningRate, int batchSize, double l1decay = 0, double l2decay = 0,
            OptimizerMethod optimizerMethod = OptimizerMethod.RMSProp, double momentum = 0.9, double rho = 0.95, double beta1 = 0.9, double beta2 = 0.999)
        {
            ...
            m_learningRateInit = learningRate;
        ...
        public void UpdateParameters(List<Data<double>> parametersAndGradients)
        {
            m_iterationCounter++;
        ...
        void InitializeAccumulators(List<Data<double>> parametersAndGradients)
        {
            for (var i = 0; i < parametersAndGradients.Count; i++)
            {
                gsumWeights.Add(new double[parametersAndGradients[i].Tensor.Data.Length]);
                ...
                {
                    xsumWeights.Add(new double[parametersAndGradients[i].Tensor.Data.Length]);
                }
            }
        }
        ...
        void UpdateLearningRate()
        {
            switch (OptimizerMethod)
            {
                case OptimizerMethod.Adam:
                    {
                        m_learningRate = (m_learningRateInit * Math.Sqrt(1.0 - Math.Pow(m_beta2, m_iterationCounter)) /
                            (1 - Math.Pow(m_beta1, m_iterationCounter)));
                    }
                    break;
                case OptimizerMethod.AdaMax:
                    {
                        m_learningRate = (m_learningRateInit / (1.0 - Math.Pow(m_beta1, m_iterationCounter)));
                    }
                    break;
                case OptimizerMethod.Nadam:
                    {
                        ...
                        m_schedule = m_scheduleNew;
                    }
                    break;
            }
        }

        void UpdateParam(int i, double[] parameters, double[] gradients, double l2Decay, double l1Decay,
            List<double[]> gsum, List<double[]> xsum)
        {
            for (var j = 0; j < parameters.Length; j++)
            {
                    ...
                    case OptimizerMethod.Sgd:
                        {
                            ...
                            {
                                parameters[j] += (-m_learningRate * gij);
                            }
                        }
                        break;
                    case OptimizerMethod.Adam:
                ...
                gradients[j] = 0.0; // zero out gradient between each iteration
            }
        }
